...
def get_model(model_name: str, data, hp: dict):
    ...
    if model_name == "baseline":
        ...
        model = LGRACE(_encoder, hp['hidden'], hp['proj_hidden']).to(device)
    elif model_name == "cgrace":
        model = CGRACE(_encoder, hp['hidden'], hp['proj_hidden']).to(device)
    elif model_name in  ["agrace", "asrcgrace"]:
    ...
    elif model_name in ["abgrl", "asrcbgrl"]:
        _predictor = MLP_Head_BGRL(hp['hidden'], hp['hidden']).to(device)
...
class CGRACE(nn.Module):
    def __init__(self, encoder: ENCODER_GRACE, hidden: int, proj_hidden: int, tau: float = 0.5):
        super(CGRACE, self).__init__()
        self.encoder: ENCODER_GRACE = encoder
        self.tau: float = tau
    ...
    def semi_loss(self, z1: torch.Tensor, z2: torch.Tensor, laplacian: torch.Tensor):
        laplacian_one = torch.where(laplacian == 0, torch.ones_like(laplacian), laplacian) # put 1 where Laplacian equal 0
        f = lambda x: - laplacian_one * torch.exp(x / self.tau)
        refl_sim = f(self.sim(z1, z1))
        between_sim = f(self.sim(z1, z2))

        return -torch.log((torch.sum(refl_sim[laplacian < 0]) + torch.sum(between_sim[laplacian < 0]))/
                          -(torch.sum(refl_sim[laplacian == 0]) + torch.sum(between_sim[laplacian == 0])))
    ...
    def loss(self, z1: torch.Tensor, z2: torch.Tensor, laplacian: torch.Tensor, mean: bool = True, batch_size: int = None):
        h1 = self.projection(z1)
        h2 = self.projection(z2)

        if batch_size is None:
            l1 = self.semi_loss(h1, h2, laplacian)
            l2 = self.semi_loss(h2, h1, laplacian)
        else:
            l1 = self.batched_semi_loss(h1, h2, batch_size)
            l2 = self.batched_semi_loss(h2, h1, batch_size)
