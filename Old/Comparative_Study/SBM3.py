...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
full_res.append(tr.runs('SBM_BGRL2+MLP', init_model, pretrain_bgrl_commu, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('deep GCN+mlp', init_model, None, data_split, evaluator, hp))
...
def init_model(data, hp):
    encoder = enc.ENCODER_BGRL([data.num_features, hp['ct_param']['num_hidden']], batchnorm=True).to(device)
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('bgrlGCN+mlp', init_model, None, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('GCA_deg+mlp', init_model, pretr.pretrain_gca, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('GCA_pr+mlp', init_model, pretr.pretrain_gca, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('GCA_evc+mlp', init_model, pretr.pretrain_gca, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
full_res.append(tr.runs('BGRL_2+mlp', init_model, pretr.pretrain_bgrl, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('BGRL_deg+mlp', init_model, pretr.pretrain_bgrl_adaptative, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('BGRL_pr+mlp', init_model, pretr.pretrain_bgrl_adaptative, data_split, evaluator, hp))
...
def init_model(data, hp):
    ...
    predictor = dec.MlpProdDecoder(hp['hiddim'], hp['hiddim']).to(device)
    return encoder, predictor
#full_res.append(tr.runs('BGRL_evc+mlp', init_model, pretr.pretrain_bgrl_adaptative, data_split, evaluator, hp))
...
df.to_csv(f'output/SBM_mlp_400_res2.csv', sep=';')
